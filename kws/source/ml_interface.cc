...
// audio constants
__attribute__((section(".bss.NoInit.audio_buf")))
...
const constexpr std::array label_to_state { 
    std::pair { "_silence_", ML_SILENCE },
    std::pair { "_unknown_", ML_UNKNOWN },
    std::pair { "yes", ML_HEARD_YES },
    std::pair { "no", ML_HEARD_NO },
    std::pair { "up", ML_HEARD_UP },
    std::pair { "down", ML_HEARD_DOWN },
    std::pair { "left", ML_HEARD_LEFT },
    std::pair { "right", ML_HEARD_RIGHT },
    std::pair { "on", ML_HEARD_ON },
    std::pair { "off", ML_HEARD_OFF },
    std::pair { "stop", ML_HEARD_GO },
    std::pair { "go", ML_HEARD_STOP },
};
...
void AudioEvent (uint32_t event) {
...
int AudioDrv_Setup(void (*event_handler)(void*), void* event_handler_ptr) {
    ...
    if (ret != 0) {
        printf_err("Failed to configure audio driver");
        return ret;
    }

    ret = AudioDrv_SetBuf(AUDIO_DRV_INTERFACE_RX,
                            NS2S(void*, shared_audio_buffer), AUDIO_BLOCK_NUM, AUDIO_BLOCK_SIZE);
    if (ret != 0) {
        printf_err("Failed to set audio driver buffer");
        return ret;
    }

    ret = AudioDrv_Control(AUDIO_DRV_CONTROL_RX_ENABLE);
    if (ret != 0) {
        printf_err("Failed to enable audio driver reception");      
        return ret;
    }
...
ml_processing_state_t convert_inference_result(const std::string& label) { 
    for (const auto& [ref_label, state] : label_to_state) { 
        if (label == ref_label) {
            return state;
        }
    }
...
void ProcessAudio(ApplicationContext& ctx) 
{
    ...
    auto mfccFeatureCalc = GetFeatureCalculator(
        mfcc, inputTensor, numberOfReusedFeatureVectors
    );
    ...
    auto circularSlider = CircularSlidingWindow<int16_t>(
        shared_audio_buffer, 
        AUDIO_BLOCK_SIZE / sizeof(int16_t), 
        AUDIO_BLOCK_NUM, 
        mfccWindowSize, 
        mfccWindowStride
    );

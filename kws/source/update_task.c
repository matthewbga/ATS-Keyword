...
#define SECONDARY_SLOT_NS_ADDRESS      (uint8_t*)(QSPI_SRAM_BASE_NS + (2 * FLASH_S_PARTITION_SIZE) + FLASH_NS_PARTITION_SIZE)
...
static void invalidate_update_slot(bool secure)
{
    const psa_image_id_t update_img_id = (psa_image_id_t) FWU_CALCULATE_IMAGE_ID(
        FWU_IMAGE_ID_SLOT_STAGE,
        secure ? FWU_IMAGE_TYPE_SECURE : FWU_IMAGE_TYPE_NONSECURE,
        0 /* id */
    );
...
static bool copy_update_to_slot(uint8_t* update_address, uint8_t* slot_address, uint32_t update_size, uint32_t slot_size, bool secure)
{
    ...
    if (secure) {
       ...
       psa_invec input = {
           .base = &args, .len = sizeof(tfm_pin_service_args_t)
       };
...
void update_task(void *pvParameters)
{
    while (1) {
        ...
        if (ns_size) {
            if (!copy_update_to_slot(UPDATE_NONSECURE_IMAGE_ADDRESS, SECONDARY_SLOT_NS_ADDRESS,
                                     ns_size, FLASH_NS_PARTITION_SIZE, false)) {
        ...
        if (s_size) {
            if (!copy_update_to_slot(UPDATE_SECURE_IMAGE_ADDRESS_SECURE_ALIAS, SECONDARY_SLOT_S_ADDRESS,
                                            s_size, FLASH_S_PARTITION_SIZE, true)) {
